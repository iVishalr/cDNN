...
int main(){
  ...
  Model(.x_train=m->x_train,.Y_train=m->Y_train,.x_cv=m->x_cv,.Y_cv=m->Y_cv,.x_test=m->x_test,.Y_test=m->Y_test,\
        .num_iter=2000,.learning_rate=3e-4,.regularization="L2",.lambda=5e-4,.optimizer="adam",.checkpoint_every=1000);
  m->fit();
