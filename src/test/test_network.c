...
int main(){
  ...
  cross_entropy_loss();
  Model(.x_train=m->x_train,.Y_train=m->Y_train,.x_cv=m->x_cv,.Y_cv=m->Y_cv,.num_iter=2000,.learning_rate=0.005);
  printComputation_Graph(m->graph);
  // m->load_model("new_model_no_bugs.t7");
  m->fit();
  m->save_model("new_model_no_bugs.t7");
  plot_train_scores();
  printComputation_Graph(m->graph);
  destroy_G(m->graph);
}