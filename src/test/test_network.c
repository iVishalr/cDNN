...
int main(){
  ...
  int x_train_dims[] = {12288,2};
  load_x_train(x_train_dims);
  int x_cv_dims[] = {12288,1};
  load_x_cv(x_cv_dims);
  int y_train_dims[] = {1,2};
  load_y_train(y_train_dims);
  int y_cv_dims[] = {1,1};
  load_y_cv(y_cv_dims);
  
  int x_test_dims[] = {12288,1};
  int y_test_dims[] = {1,1};
  ...
  Model(.x_train=m->x_train,.Y_train=m->Y_train,.x_cv=m->x_cv,.Y_cv=m->Y_cv,.x_test=m->x_test,.Y_test=m->Y_test,.num_iter=2000,.learning_rate=0.007,.regularization="L2",.lambda=5e-4);
  // m->fit();
